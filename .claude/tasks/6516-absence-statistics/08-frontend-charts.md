...
- Visuelles Review nach Fertigstellung: die Prozentzahl im Ring hat sich mit dem Ring selbst überschnitten, weil das Label die volle (dreistellige Nachkomma-)Präzision aus `VacationDaysTaken` unformatiert übernommen hat. Jetzt im `barLabels`-Formatter auf eine Ganzzahl gerundet, dazu `hollow.size` von 34 % auf 45 % und `offsetX` von -8 auf 0 — mehr Platz für das Label, unabhängig von der Rundung. Außerdem bekam das Kuchendiagramm `tooltip.followCursor: true`, damit es sich wie das Balkendiagramm verhält.
- Die Rundung allein hat die Überschneidung nicht behoben: `barLabels` positioniert den Text grundsätzlich neben dem Bogen (gedacht für mehrere Ringe wie beim Sick-Note-Gauge, wo jeder Ring sein eigenes Label direkt daneben braucht), nicht mittig — bei nur einem Ring lief das Label quer über den Bogen statt sauber daneben. Ersetzt durch das dafür vorgesehene `dataLabels.value` (mittig im hohlen Zentrum), `hollow.size` dafür von 45 % auf 62 % erhöht.
