...
**Gauge** — `type: "radialBar"` mit zwei Serien (Stand heute, Prognose 31.12.), `startAngle: 0`, `endAngle: 270`, `barLabels` aktiv mit Prozentformatierung — dieselben `plotOptions` wie der Gauge in `sick-notes-statistics.js`. Bei vergangenen Jahren liefert das Model nur einen Wert; das Bundle darf daraus keine zweite Serie erfinden.

**Stylesheet** — Container- und Diagrammhöhen reservieren, damit die Seite beim Nachladen des Bundles nicht springt; `sick-note-statistics.css` erklärt in seinen Kommentaren, warum und was mit den Konstanten im JS synchron zu halten ist. Dazu Tooltip-Zeilenlayout und das zweispaltige Raster für Kuchen und Gauge.
...
- Gauge mit zwei Werten erzeugt zwei Serien, Gauge mit einem Wert genau eine
