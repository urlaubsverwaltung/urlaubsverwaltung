...
class BlackoutPeriodApiControllerTest {
    ...
    @Test
    void personsBlackoutPeriodsExpandsPeriodIntoDaysWhenPeriodIsWithinRequestedRange() throws Exception {
        ...
        perform(get("/api/persons/42/blackout-periods")
            ...
            .andExpect(status().isOk())
            .andExpect(jsonPath("$.blackoutPeriods", hasSize(2)))
            .andExpect(jsonPath("$.blackoutPeriods[0].date", is("2026-06-04")))
            .andExpect(jsonPath("$.blackoutPeriods[1].date", is("2026-06-05")));
    }

    @Test
    void personsBlackoutPeriodsBadRequestForUnknownPerson() throws Exception {

        when(personService.getPersonByID(42L)).thenReturn(Optional.empty());
