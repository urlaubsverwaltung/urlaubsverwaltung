...
class PersonPermissionsMapperTest {
    ...
    @Test
    void ensureInactivePermissionMapsToNoRoles() {
        final List<Role> roles = mapPermissionsDtoToRole(List.of(PersonPermissionsRoleDto.INACTIVE));
    ...
    void ensureRolesMappingToPermissionsContainsAllPermissions() {
        ...
        final List<PersonPermissionsRoleDto> permissionsWithoutInactive = Arrays.stream(PersonPermissionsRoleDto.values())
            .filter(permission -> !permission.equals(PersonPermissionsRoleDto.INACTIVE))
            .toList();
        assertThat(permissions).containsExactlyElementsOf(permissionsWithoutInactive);
    }

    @Test
    void ensurePersonWithoutUserRoleIsMappedToInactivePermission() {
        final Person person = new Person();
        person.setPermissions(List.of());

        final PersonPermissionsDto personPermissionsDto = mapToPersonPermissionsDto(person);
        assertThat(personPermissionsDto.getPermissions()).containsExactly(PersonPermissionsRoleDto.INACTIVE);
        assertThat(personPermissionsDto.getIsInactive()).isTrue();
    }

    @Test
    void ensurePersonWithUserRoleIsNotMappedToInactivePermission() {
        final Person person = new Person();
        person.setPermissions(List.of(Role.USER, Role.OFFICE));

        final PersonPermissionsDto personPermissionsDto = mapToPersonPermissionsDto(person);
        assertThat(personPermissionsDto.getPermissions()).containsExactly(PersonPermissionsRoleDto.USER, PersonPermissionsRoleDto.OFFICE);
        assertThat(personPermissionsDto.getIsInactive()).isFalse();
    }
}
