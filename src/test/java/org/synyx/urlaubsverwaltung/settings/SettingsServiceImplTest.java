...
class SettingsServiceImplTest {
    ...
    @Test
    void ensureGetNewEmptySettingIfNothingIsSaved() {
        when(settingsRepository.findAll()).thenReturn(List.of());
        when(settingsRepository.save(any(Settings.class))).thenAnswer(returnsFirstArg());

        final Settings actualSettings = sut.getSettings();
        assertThat(actualSettings).isInstanceOf(Settings.class);
    }
}
