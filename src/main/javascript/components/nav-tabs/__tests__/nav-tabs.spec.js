...
describe("nav-tabs", function () {
  ...
  it("updates look and feel after nav-tab is clicked", async function () {
    ...
    div.innerHTML = `
      <uv-nav-tabs>
        ...
        <li class="border-b-4 border-transparent" data-content="#content-bbb">
          <a href="#content-bbb" class="text-zinc-400">
            BBB
          </a>
        </li>
      </uv-nav-tabs>
    ...
    expect(navTabA.classList.contains("border-zinc-200")).toBeFalsy();
    expect(navTabA.classList.contains("border-transparent")).toBeTruthy();
    expect(navTabALink.classList.contains("text-black-almost")).toBeFalsy();
    expect(navTabALink.classList.contains("text-zinc-400")).toBeTruthy();
    expect(contentA.hasAttribute("hidden")).toBeTruthy();

    expect(navTabB.dataset.active).toBeDefined();
    expect(navTabB.classList.contains("border-zinc-200")).toBeTruthy();
    expect(navTabB.classList.contains("border-transparent")).toBeFalsy();
    expect(navTabBLink.classList.contains("text-black-almost")).toBeTruthy();
    expect(navTabBLink.classList.contains("text-zinc-400")).toBeFalsy();
  ...
  it("does not update look and feel after current active nav-tab is clicked", function () {
    ...
    expect(navTabA.classList.contains("border-zinc-200")).toBeTruthy();
    expect(navTabA.classList.contains("border-transparent")).toBeFalsy();
    expect(navTabALink.classList.contains("text-black-almost")).toBeTruthy();
    expect(navTabALink.classList.contains("text-zinc-400")).toBeFalsy();
    expect(contentA.hasAttribute("hidden")).toBeFalsy();

    expect(navTabB.dataset.active).not.toBeDefined();
    expect(navTabB.classList.contains("border-zinc-200")).toBeFalsy();
    expect(navTabB.classList.contains("border-transparent")).toBeTruthy();
    expect(navTabBLink.classList.contains("text-black-almost")).toBeFalsy();
    expect(navTabBLink.classList.contains("text-zinc-400")).toBeTruthy();
  ...
  it("replaces the URL when nav-tab is clicked", function () {
    ...
    expect(globalThis.history.replaceState).toHaveBeenLastCalledWith(
      undefined,
      undefined,
      "http://localhost:3000/#content-bbb",
    );
    ...
    expect(globalThis.history.replaceState).toHaveBeenLastCalledWith(
      undefined,
      undefined,
      "http://localhost:3000/#content-aaa",
    );
  });
