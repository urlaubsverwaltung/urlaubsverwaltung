...
class SickNoteDataCollectionService {
    ...
    SickNoteBackupDTO collectSickNotes(List<Person> allPersons, LocalDate from, LocalDate to) {
        final List<SickNoteTypeDTO> sickNoteTypeDTOS = sickNoteTypeService.getSickNoteTypes().stream().map(sickNoteType -> new SickNoteTypeDTO(sickNoteType.getId(), SickNoteTypeCategoryDTO.valueOf(sickNoteType.getCategory().name()), sickNoteType.getMessageKey())).toList();
        final List<SickNoteDTO> sickNoteDTOs = allPersons.stream().map(person -> sickNoteService.getByPersonAndPeriod(person, from, to).stream().map(sickNote -> {
            ...
            final List<SickNoteCommentDTO> sickNoteCommentDTOs = sickNoteCommentService.getCommentsBySickNote(sickNote).stream().map(sickNoteComment -> new SickNoteCommentDTO(sickNoteComment.getDate(), sickNoteComment.getText(), SickNoteCommentActionDTO.valueOf(sickNoteComment.getAction().name()), sickNoteComment.getPerson().getUsername())).toList();
            return new SickNoteDTO(sickNote.getId(), sickNote.getPerson().getUsername(), sickNote.getApplier().getUsername(), sickNote.getSickNoteType().getId(), sickNote.getStartDate(), sickNote.getEndDate(), DayLengthDTO.valueOf(sickNote.getDayLength().name()), sickNote.getAubStartDate(), sickNote.getAubEndDate(), sickNote.getLastEdited(), sickNote.getEndOfSickPayNotificationSend(), SickNoteStatusDTO.valueOf(sickNote.getStatus().name()), sickNoteCommentDTOs, sickNoteExtensionHistoryDTOs);
