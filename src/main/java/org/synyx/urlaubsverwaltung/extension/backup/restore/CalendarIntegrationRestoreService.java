...
class CalendarIntegrationRestoreService {
    ...
    CalendarIntegrationRestoreService(CalendarIntegrationSettingsImportService calendarIntegrationSettingsImportService, AbsenceMappingImportService absenceMappingImportService) {
    ...
    private static Long resolveIdOfImportedAbsence(List<ImportedIdTuple> tuplesOfCreatedEntities, AbsenceMappingDTO absenceMapping, AbsenceMappingType vacation) {
        return tuplesOfCreatedEntities.stream().filter(tuple -> tuple.idOfBackup().equals(absenceMapping.absenceId())).map(ImportedIdTuple::idOfRestore).findFirst().orElseThrow(() -> new IllegalStateException("No absence with type=%s for id=%s found - going to skip importing absenceMapping!".formatted(vacation.name(), absenceMapping.absenceId())));
    }
