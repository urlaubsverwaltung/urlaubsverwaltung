 ...
 *
 * <p>Who is responsible for the person of the application is answered by an {@link ApplicationResponsibility}, which
 * hits the database. Cheap checks are therefore evaluated first and an instance is meant to live no longer than one
 * request.
 */
public class ApplicationForLeavePermissions {

    private final ApplicationResponsibility responsibility;
    private final Person signedInUser;
    private final Application application;

    ApplicationForLeavePermissions(ApplicationResponsibility responsibility, Person signedInUser, Application application) {
    ...
    public boolean isAllowedToAllowWaiting() {
        return application.hasStatus(WAITING)
            ...
            || (isSecondStageAuthorityOfPerson() && isNotOwnApplication())
            || (isDepartmentHeadOfPerson() && isNotOwnApplication() && !isPersonSecondStageAuthorityOfSignedInUser()));
    }
    ...
    public boolean isAllowedToAllowTemporaryAllowed() {
        return application.hasStatus(TEMPORARY_ALLOWED)
            && (signedInUser.hasRole(BOSS) || (isSecondStageAuthorityOfPerson() && isNotOwnApplication()));
    }
    ...
    public boolean isAllowedToRefer() {
        return (application.hasStatus(WAITING) || application.hasStatus(TEMPORARY_ALLOWED))
            ...
            || ((isDepartmentHeadOfPerson() || isSecondStageAuthorityOfPerson()) && isNotOwnApplication()));
    }
    ...
    public boolean isAllowedToStartCancellationRequest() {
        return (application.hasStatus(ALLOWED) || application.hasStatus(TEMPORARY_ALLOWED) || application.hasStatus(ALLOWED_CANCELLATION_REQUESTED))
            && requiresApprovalToCancel()
            && (isOwnApplication() || signedInUser.hasRole(BOSS) || isDepartmentHeadOfPerson() || isSecondStageAuthorityOfPerson())
            && !isAllowedToCancel();
    }
    ...
    private boolean isDepartmentHeadOfPerson() {
        return responsibility.isDepartmentHeadOf(application.getPerson());
    }

    private boolean isSecondStageAuthorityOfPerson() {
        return responsibility.isSecondStageAuthorityOf(application.getPerson());
    }

    private boolean isPersonSecondStageAuthorityOfSignedInUser() {
        return responsibility.isSecondStageAuthorityOfSignedInUser(application.getPerson());
    }
}
