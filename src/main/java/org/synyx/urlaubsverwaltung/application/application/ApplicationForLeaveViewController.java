...
class ApplicationForLeaveViewController implements HasLaunchpad {
    ...
    private void prepareApplicationModels(Model model, Locale locale) {
        ...
        final List<SickNoteDto> otherSickNotes = sickNoteService.getForStatesAndPersons(List.of(SickNoteStatus.SUBMITTED), getPersonsForRelevantSubmittedSickNotes(signedInUser)).stream().map(SickNoteDto::new).toList();
