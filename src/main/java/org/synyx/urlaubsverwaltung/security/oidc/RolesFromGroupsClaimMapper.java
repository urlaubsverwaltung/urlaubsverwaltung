...
class RolesFromGroupsClaimMapper implements RolesFromClaimMapper {
    ...
    public List<GrantedAuthority> mapClaimToRoles(final Map<String, Object> claims) {
        ...
        if (properties.isAuthorityCheckEnabled() && !claims.containsKey(groupClaim.getClaimName())) {
            LOG.error("User with sub '{}' has not required permission '{}' in '{}' to access urlaubsverwaltung! The claim '{}' is missing!", claims.get(SUB), neededResourceAccessRole, groupClaim.getClaimName(), claims.get("groups"));
            throw new MissingClaimAuthorityException(format("User with sub '%s' has not required permission '%s' in '%s' to access urlaubsverwaltung! The claim '%s' is missing!", claims.get(SUB), neededResourceAccessRole, groupClaim.getClaimName(), claims.get("groups")));
        }
